...
export class GetPredictions {
  ...
  async getPredictionFeeds(address: string) {
    if (address === "") {
      console.log("address is tmpty");
      return;
    }
    ...
    const res = await _Contract.getPredictionFeed(address);
    console.log("getPredictionFeed suc: ", res);
  }
}
